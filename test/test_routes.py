...
def test_index_route(client):
    ...
    assert (
        "Bienvenue sur le GUDLFT Portal" in text
        or "Welcome to the GUDLFT Portal" in text
    )
...
def test_welcome_route_invalid_email_flash(client):
    ...
    assert "Liste des Clubs" in text or "Bienvenue sur le GUDLFT Portal" in text
...
def test_club_route_without_login(client):
    ...
    text = response.get_data(as_text=True)
    # The app currently redirects to the clubs list page when not logged in
    assert "Liste des Clubs" in text or "Bienvenue sur le GUDLFT Portal" in text
...
def test_club_route_requires_login(client):
    ...
    assert (
        "Liste des Clubs" in data_text
        or "Bienvenue sur le GUDLFT Portal" in data_text
...
def test_competition_detail_without_login(client):
    ...
    assert "Vous devez être connecté" in text or "Competition" in text
...
def test_competition_detail_invalid_name(client, monkeypatch):
    ...
    assert "Compétition introuvable" in response.get_data(as_text=True)
...
def test_purchase_places_more_than_available(client, monkeypatch):
    ...
    response = client.post("/purchasePlaces", data=data, follow_redirects=True)
    assert "demandées" in response.get_data(as_text=True)
...
def test_purchase_places_more_than_points(client, monkeypatch):
    ...
    # Also ensure loadClubs and loadCompetitions return expected structures
    monkeypatch.setattr(
        "server.loadClubs",
        lambda: [{"name": "Simply Lift", "email": "john@simplylift.co", "points": "5"}],
    )
    monkeypatch.setattr(
        "server.loadCompetitions",
        lambda: [
    ...
    response = client.post("/purchasePlaces", data=data, follow_redirects=True)
    assert "Pas assez de pts" in response.get_data(as_text=True)
...
def test_purchase_places_more_than_limit(client, monkeypatch):
    ...
    response = client.post("/purchasePlaces", data=data, follow_redirects=True)
    assert "Vous ne pouvez pas réserver plus de 12 places" in response.get_data(
        as_text=True
    )
...
def test_competition_detail_route(client, monkeypatch):
    ...
    assert response.status_code == 200
    assert "Competition" in response.get_data(as_text=True)
