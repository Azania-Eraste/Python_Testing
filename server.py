...
def create_app(test_config=None):
    ...
    # Si on fournit une config de test
    ...
    def purchasePlaces():
        ...
        competitions = loadCompetitions()
        print(
            f"club: {club_name} | competition: {comp_name} | places: {places_requested}"
        )
        ...
        if places_requested > available_places:
            flash(f"demandées({places_requested}) > disponibles({available_places})!")
        elif places_requested > int(club["points"]):
            flash(
                f"Pas assez de pts({club['points']}) pour les {places_requested} plcs !"
            )
            ...
            flash(
                f"✅ Réservation réussie : {places_requested} place(s) pour {comp_name}."
            )
